...
6. **`RecommendController.getRecommendation`** (`saarathi-backend/src/recommend/recommend.controller.ts`) validates this body against a Zod schema (`RecommendRequestSchema`) and delegates to **`RecommendService.getRecommendation`** (`saarathi-backend/src/recommend/recommend.service.ts`), which routes it to either **`RecommendSingleLegService`** (`saarathi-backend/src/recommend/recommend-single-leg.service.ts`) or **`RecommendMultiCityService`** (`saarathi-backend/src/recommend/recommend-multi-city.service.ts`).
...
12. **`computeCounterfactuals(ranked, basePref, routeFlights, opts)`** (`core/counterfactuals.ts:45-174`) — note this is called with `basePref` (the *unperturbed* preferences), not `perturbedPref`, at `recommend-single-leg.service.ts` — see §10 for why that's worth double-checking. Produces 8 counterfactuals (3 price-threshold, 5 toggle-based); worked arithmetic in §4.
...
**If deleted:** the entire `cities: [...]` request mode breaks — `RecommendMultiCityService`'s multi-city branch (`recommend-multi-city.service.ts`) calls this directly.
...
| Add a field to the `/api/recommend` response | `saarathi-backend/src/core/types.ts` (`RecommendResponse` or the relevant sub-interface) → wherever it's computed (likely `ranking.ts`, `confidence.ts`, or a new `core/` module) → `recommend/recommend-single-leg.service.ts` or `recommend/recommend-multi-city.service.ts` (both build the response object separately — easy to update one and forget the other) → `saarathi-frontend/core/types.ts` (manual mirror) → the consuming component. |
