...
6. **`RecommendController.getRecommendation`** (`saarathi-backend/src/recommend/recommend.controller.ts`) validates this body against a Zod schema (`RecommendRequestSchema`) and delegates to **`RecommendService.getRecommendation`** (`saarathi-backend/src/recommend/recommend.service.ts`).
...
15. The service assembles a `TraceStage[]` (7 stages) and returns the full `RecommendResponse`.
...
**If deleted:** the entire `cities: [...]` request mode breaks — `RecommendService`'s multi-city branch (`recommend.service.ts`) calls this directly.
...
|---|---|---|---|---|
| **NestJS** | Opinionated Node.js server framework (decorators, dependency injection, modules) built on Express. | Gives the scoring engine a real HTTP boundary and DI container without hand-rolling routing/validation; `@Controller`/`@Module`/`@Injectable` keep controllers as thin validation wrappers and services like `RecommendService` as the orchestrators. | `src/main.ts`, `src/app.module.ts`, `src/recommend/recommend.controller.ts`, `src/recommend/recommend.service.ts`, `src/users/users.controller.ts` | A plain Express app, or (per the original plan) no separate backend at all — Next.js Route Handlers directly in the frontend. |
...
| Add a field to the `/api/recommend` response | `saarathi-backend/src/core/types.ts` (`RecommendResponse` or the relevant sub-interface) → wherever it's computed (likely `ranking.ts`, `confidence.ts`, or a new `core/` module) → `recommend/recommend.service.ts` (both the single-leg and multi-city branches build the response object separately — easy to update one and forget the other) → `saarathi-frontend/core/types.ts` (manual mirror) → the consuming component. |
