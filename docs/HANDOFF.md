...
6. **`RecommendController.getRecommendation`** (`saarathi-backend/src/recommend/recommend.controller.ts`) validates this body against a Zod schema (`RecommendRequestSchema`) and delegates to **`RecommendService.getRecommendation`** (`saarathi-backend/src/recommend/services/recommend.service.ts`), which routes it to either **`RecommendSingleLegService`** (`saarathi-backend/src/recommend/services/recommend-single-leg.service.ts`) or **`RecommendMultiCityService`** (`saarathi-backend/src/recommend/services/recommend-multi-city.service.ts`).
7. **`getStore()`** (`core/data.ts`) returns the in-memory Maps. `store.users.get('U01')`.
...
12. **`computeCounterfactuals(ranked, basePref, routeFlights, opts)`** (`core/counterfactuals.ts:45-174`) — note this is called with `basePref` (the *unperturbed* preferences), not `perturbedPref`, at `services/recommend-single-leg.service.ts` — see §10 for why that's worth double-checking. Produces 8 counterfactuals (3 price-threshold, 5 toggle-based); worked arithmetic in §4.
...
15. The selected recommendation service assembles a `TraceStage[]` (7 stages) and returns the full `RecommendResponse`.
...
**If deleted:** the entire `cities: [...]` request mode breaks — `RecommendMultiCityService`'s multi-city branch (`services/recommend-multi-city.service.ts`) calls this directly.
...
|---|---|---|---|---|
| **NestJS** | Opinionated Node.js server framework (decorators, dependency injection, modules) built on Express. | Gives the scoring engine a real HTTP boundary and DI container without hand-rolling routing/validation; `@Controller`/`@Module`/`@Injectable` keep controllers as thin validation wrappers and services like `RecommendService`, `RecommendSingleLegService`, and `RecommendMultiCityService` as the orchestrators. | `src/main.ts`, `src/app.module.ts`, `src/recommend/recommend.controller.ts`, `src/recommend/services/recommend.service.ts`, `src/recommend/services/recommend-single-leg.service.ts`, `src/recommend/services/recommend-multi-city.service.ts`, `src/users/users.controller.ts` | A plain Express app, or (per the original plan) no separate backend at all — Next.js Route Handlers directly in the frontend. |
...
| Add a field to the `/api/recommend` response | `saarathi-backend/src/core/types.ts` (`RecommendResponse` or the relevant sub-interface) → wherever it's computed (likely `ranking.ts`, `confidence.ts`, or a new `core/` module) → `recommend/services/recommend-single-leg.service.ts` or `recommend/services/recommend-multi-city.service.ts` (both build the response object separately — easy to update one and forget the other) → `saarathi-frontend/core/types.ts` (manual mirror) → the consuming component. |
