## 1. Boot-Up Sequence (Data Caching)                                                                                                                                                      
  ...
  • Files: recommend.controller.ts, recommend.service.ts                                                                                                                                     
  • Flow: The controller validates the request payload and delegates the execution to the `RecommendService`. The service coordinates the multi-stage preference inference, filtering, scoring, counterfactual algebra, and LLM explanation pipeline. Below is the step-by-step function call stack for single-leg and multi-city requests.                                                                                                                                                           
  ──────                                                                                                                                                                                     
  ...
  The `RecommendService` collects the outputs, builds a structured execution trace (representing the stages shown in the frontend's debugging panel), and returns the response object. The controller then returns the JSON payload back to the Next.js frontend client:                                                                                                                                                                   
